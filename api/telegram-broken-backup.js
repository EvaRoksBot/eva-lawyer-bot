...
// Check if we're in demo mode (invalid or missing token)
const TELEGRAM_BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN;
const DEMO_MODE = !TELEGRAM_BOT_TOKEN || 
                  TELEGRAM_BOT_TOKEN === 'invalid' || 
                  TELEGRAM_BOT_TOKEN === 'your_bot_token_here' ||
                  TELEGRAM_BOT_TOKEN.length < 40;

console.log(`🤖 Bot mode: ${DEMO_MODE ? 'DEMO' : 'PRODUCTION'}`);
console.log(`🔑 Token status: ${TELEGRAM_BOT_TOKEN ? 'present' : 'missing'}`);
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    return res.status(200).json({
      status: 'ok',
      message: 'Eva Lawyer Bot Final API is running',
      ...
      note: DEMO_MODE ? 'Demo mode active - provide valid TELEGRAM_BOT_TOKEN to activate production mode' : 'Production mode active'
    });
  }
  ...
  try {
    ...
    if (DEMO_MODE) {
      ...
      // Here would be the real bot logic with actual Telegram API calls
